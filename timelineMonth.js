...
// VARIABLES
let year = ""
// creation variables pour selectionner Pole et period à viser
let periodSelected = []
let poleSelected = ""
...
function monthRow(){
    ...
    periodSelected.map(element => {
        // Calculer le nombre de jours de mois
        daysNbr = getDaysNbr(element) 
        // Afficher le mois
        let month = createMyElement('th', element)
...
function showTimelinePole(e) { 
    ...
    {
        // Creer le tableau de mois selectionnés
        period.forEach((elem) => {
            elem.classList.contains('active') && periodSelected.push(elem.innerHTML)
        })
        // Si le period de l'année n'a pas été selectionné afiicher toute l'année
        if(!periodSelected.length){
            period.forEach((elem) => {
                periodSelected.push(elem.innerHTML)
            })
        }
        ...
        departementTable.classList.add('data-table')
...
function showDays(startDay, daysNbr, place, month, personStart, personEnd) {  
    let i = 0
    while(i < daysNbr){
        let cell;
        let index = ((startDay+i)%7)
        // Reperer la date correspondente au jour de la semaine à afficher
        let date = new Date(`${year}`,`${months.indexOf(month)}`,`${i+1}`)
        // Creation case date
...
function getStartDay(month) {
    return new Date( year, months.indexOf(month) ).getDay()
}

// Calculer le nombre de jours de mois
function getDaysNbr(month) {
...
// Selecteur de périod de l'année
let period = document.querySelectorAll('.title')
period.forEach((element) => {
    element.addEventListener('click', selectePeriod)
})
